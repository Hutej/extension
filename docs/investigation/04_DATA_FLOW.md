...
> How data moves through WebMorph: state changes, storage, AI context, serialization/deserialization.
...
```
            ...
            │  stamp data-wm-c →   │  stamps CLUSTER_ATTR (data-wm-c) on each cluster rep
            ...
            ┌──────────▼──────────┐
            │  applyStyleEverywhere│  execute/index.ts:19  <style id=webmorph-style>.textContent = css
            │  + applyInlineBackstop (head + every open shadow root)
            └──────────┬──────────┘
                       │  DOM now mutated (style injected, data-wm-c stamped, v1: ops applied)
            ┌──────────▼──────────┐
...
1. **`data-wm-c="<handle>"` attribute** stamped on each cluster rep (`perceive:523`). This is the identity surface.
2. **`<style id=webmorph-style>`** created/appended to `<head>` and every open shadow root (`execute:19-28`). `textContent` holds the compiled CSS.
3. **`data-wm-grid` attribute** (v2) on the NCA, intermediates, full-width, proxies (`solve:491,511,529,547`) — targeting for grid CSS.
...
6. **`webmorphRunInFlight`** sessionStorage flag (`content.ts:407`) — popup reads it to show "in progress".
7. **`data-webmorph-applied`/`data-webmorph-failed`** on `<html>` (`content.ts:133-140`).
...
| `webmorphRunInFlight` | `content.ts:407` (sessionStorage actually — `content.ts` uses a session flag) | `popup/main.ts:148` | in-progress indicator |
...
- `handle` (the `data-wm-c` value, e.g. `c1a2b3`)
...
- **Spec → CSS:** `compileSpec` (`compile/index.ts`) → `expandIntents` → `buildDeclarations` → laws → a CSS string. The CSS is keyed on `[data-wm-c="<handle>"]` (v1) or `buildSelector`/`[data-wm-grid]` (v2).
...
```
perceive:523  stamp  data-wm-c="<handle>"  ──►  serialized handle in prompt
                                                     │
                                                     ▼  (model emits the same handle)
compile:302   rule.target = handle  ──►  [data-wm-c="<handle>"]  ──►  CSS matches the stamped node
                                                     │
                                                     ▼  (framework re-renders the node)
                                            data-wm-c GONE  ──►  CSS stops matching
                                                     │
                                                     ▼  (MutationObserver startDefense re-stamps)
                                            ... if it catches it in time
```
**100% of v1 CSS is keyed on the runtime-injected `data-wm-c` attribute** (`perceive:503`). `solve.ts:23` claims "data-wm-c is a DEBUG label only; nothing in emitted CSS depends on it" — **that is false for v1** (verified). v2 swaps to `buildSelector`/`data-wm-grid` but those are also injected attributes with the same survival problem.
...
- **`data-wm-c` stamps** inside shadow roots can't be cleared by `clearHandles` (`perceive:1115` uses `document.querySelectorAll` — light tree only) → stale stamps leak across runs.
