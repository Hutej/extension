...
> Beginner-friendly architecture overview of the WebMorph browser extension.
> Every claim cites `file:line` or a function name. Speculation is marked **UNVERIFIED**.

## What is WebMorph?

WebMorph is a **Chrome MV3 browser extension** built with the [WXT](https://wxt.dev) framework that reshapes *any* website from a plain-English prompt — entirely inside the user's browser. It never calls the site's backend. The user types a vibe (e.g. "1970s sci-fi paperback cover"), and WebMorph re-skins and (in the v2 path) re-lays-out the page using a hosted AI model plus a deterministic CSS compiler.

- **Repo root:** `D:\WebMorph`
...
WebMorph has two pipelines selected by a flag `layoutCompiler = 'v1' | 'v2'` (default **v1**), set via the `WM_LAYOUT_COMPILER` env var (`wxt.config.ts:26`, `config/index.ts:11`) and a popup dev toggle. The two are *meant* to be separate, but in practice the orchestrator (`content.ts`) is full of `if (v2)` branches — see `07_ARCHITECTURE_ANALYSIS.md`.

- **v1 (legacy, current default):** `perceive → reason (StyleSpec) → compile (expand+laws) → verify → repair → apply`. Emits per-element CSS keyed on a runtime-injected `data-wm-c` attribute. Can also emit structural DOM ops (move/remove/reorder/wrap).
...
| **Perception** | `core/perceive/index.ts` (1489), `core/perceive/semantic.ts` | Walk DOM → cluster → stamp `data-wm-c` → role-classify → serialize |
...
Think of WebMorph as a four-stage factory:

1. **Look** (`perceive`): "Walk the page, group similar boxes, give each group a name (`data-wm-c`) and a job title (role: masthead/nav/main/footer/ad)."
...
The product's correctness rests on two fragile pillars: (1) **identity** — every CSS rule targets a runtime-injected `data-wm-c` attribute that disappears the moment a framework re-renders the node; and (2) **perception** — which walks shadow roots but reads them back with light-tree `document.querySelector`, silently classifying all web-component content (YouTube) as empty voids. These are verified defects, not speculation. The detailed evidence is in the rest of this documentation set.
