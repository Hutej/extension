...
export const PLANNER_SYSTEM_PREFIX = [
  ...
  '  {"kind":"style","rules":[{"target":{"targetRef"},"surface":"element"|"before"|"after","state":"none"|"hover"|"focus-visible"|"focus-within"|"active"|"checked"|"disabled","declarations":[{"property","value","priority"?:"important"|"normal"}],"conditions":[...]}],"keyframes"?:[...]}',
  ...
  '- Style operations are INTENTIONAL OVERRIDES: priority defaults to important — OMIT it. The generated selectors are zero-specificity (:where) so the site\'s own rules otherwise win and the batch rolls back at verification. Use priority "normal" ONLY to deliberately defer to the site\'s own rule on that element.',
