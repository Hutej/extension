...
// ── F5: Movable element capability ───────────────────────────────────
// Transform-based drag + keyboard movement. No DOM mutation — only
// transform: translate(). Fully reversible (clear transforms). Invisible
// to framework MutationObservers. Bounded to viewport. Keyboard is a
// first-class path (Arrow keys + Shift for larger steps). Only elements
// the design intent marked `movable` (CSS --rv-movable: 1) become movable.

let movableCleanup: (() => void) | null = null;

function startMovableHandlers(): void {
  stopMovableHandlers();
...
function stopMovableHandlers(): void {
